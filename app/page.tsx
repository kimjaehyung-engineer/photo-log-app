...
export default function Home() {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      // Set column widths for A4 width
      worksheet.columns = [
        { width: 12 }, // A (Label)
        { width: 68 }, // B (Content/Image)
      ];
      ...
      worksheet.getRow(2).values = ["공사명", projectName || "프로젝트 미지정"];
      ...
      for (let i = 0; i < photos.length; i++) {
        ...
        worksheet.mergeCells(`A${currentRow}:B${currentRow + 10}`);
        const imageCell = worksheet.getCell(`A${currentRow}`);
        imageCell.border = {
          top: { style: 'medium' },
          left: { style: 'medium' },
          right: { style: 'medium' },
        ...
        try {
          ...
          // Target Area Dimensions in Pixels
          // Precise calculation based on column widths A(12) and B(68)
          // Average pixel per unit is approx 7.5
          const totalWidthPx = (12 + 68) * 7.5; 
          const totalHeightPx = 11 * 20; // 11 rows * 20px
          
          const scale = Math.min(totalWidthPx / imgWidth, totalHeightPx / imgHeight);
        ...
        const labelStyle = { 
          font: { bold: true }, 
          fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F2F2F2' } } as ExcelJS.Fill,
        ...
        const contentStyle = {
          alignment: { vertical: 'middle', wrapText: true } as ExcelJS.Alignment,
          border: { 
            right: { style: 'medium' }, 
            bottom: { style: 'thin' },
            top: { style: 'thin' }
